...
var _require = require('../lib/parsers'),
    dashedToCamelCase = _require.dashedToCamelCase;

var dashedProperties = [].concat(_toConsumableArray(require('../lib/allProperties')), _toConsumableArray(require('../lib/allExtraProperties')));
var allowedProperties = dashedProperties.map(dashedToCamelCase);
var implementedProperties = Array.from(require('../lib/implementedProperties')).map(dashedToCamelCase);
var invalidProperties = implementedProperties.filter(function (property) {
  return !allowedProperties.includes(property);
});
module.exports = {
  'Verify Has Only Valid Properties Implemented': function VerifyHasOnlyValidPropertiesImplemented(test) {
    test.expect(1);
    test.ok(invalidProperties.length === 0, invalidProperties.length + ' invalid properties implemented: ' + Array.from(invalidProperties).join(', '));
    test.done();
  },
  'Verify Has All Properties': function VerifyHasAllProperties(test) {
  ...
  'Verify Has All Dashed Properties': function VerifyHasAllDashedProperties(test) {
    var style = new cssstyle.CSSStyleDeclaration();
  ...
  'Float should be valid property for `.setProperty()`': function FloatShouldBeValidPropertyForSetProperty(test) {
    var style = new cssstyle.CSSStyleDeclaration();
    test.expect(2);
  ...
  'Make sure flex-shrink works': function MakeSureFlexShrinkWorks(test) {
    var style = new cssstyle.CSSStyleDeclaration();
    test.expect(3);
  ...
  'Make sure flex-grow works': function MakeSureFlexGrowWorks(test) {
    var style = new cssstyle.CSSStyleDeclaration();
    test.expect(2);
  ...
  'Make sure flex-basis works': function MakeSureFlexBasisWorks(test) {
    var style = new cssstyle.CSSStyleDeclaration();
    test.expect(5);
  ...
  'Make sure shorthand flex works': function MakeSureShorthandFlexWorks(test) {
    var style = new cssstyle.CSSStyleDeclaration();
    test.expect(19);
