...
var validCharacters = /[^a-zA-Z0-9_-]/g;

function getComponentName(state) {
  if (!state.filePath) return 'SvgComponent';
  var pascalCaseFileName = (0, _camelcase["default"])(_path["default"].parse(state.filePath).name.replace(validCharacters, ''), {
    pascalCase: true
  });
  return "Svg".concat(pascalCaseFileName);
}
