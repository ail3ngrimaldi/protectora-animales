...
function () {
  ...
  _createClass(Range, [{
    ...
    value: function parseRange(range) {
      ...
      }).join(' ').split(/\s+/) // in loose mode, throw out any that are not valid comparators
...
var replaceTilde = function replaceTilde(comp, options) {
  ...
  return comp.replace(r, function (_, M, m, p, pr) {
    ...
    if (isX(M)) {
      ...
      ret = ">=".concat(M, ".0.0 <").concat(+M + 1, ".0.0");
    } else if (isX(p)) {
      // ~1.2 == >=1.2.0 <1.3.0
      ret = ">=".concat(M, ".").concat(m, ".0 <").concat(M, ".").concat(+m + 1, ".0");
    } else if (pr) {
      debug('replaceTilde pr', pr);
      ret = ">=".concat(M, ".").concat(m, ".").concat(p, "-").concat(pr, " <").concat(M, ".").concat(+m + 1, ".0");
    } else {
      // ~1.2.3 == >=1.2.3 <1.3.0
      ret = ">=".concat(M, ".").concat(m, ".").concat(p, " <").concat(M, ".").concat(+m + 1, ".0");
    }
  ...
  return comp.replace(r, function (_, M, m, p, pr) {
    ...
    if (isX(M)) {
      ...
      if (M === '0') {
        ...
        ret = ">=".concat(M, ".").concat(m, ".0 <").concat(+M + 1, ".0.0");
      }
    } else if (pr) {
      ...
      if (M === '0') {
        if (m === '0') {
          ret = ">=".concat(M, ".").concat(m, ".").concat(p, "-").concat(pr, " <").concat(M, ".").concat(m, ".").concat(+p + 1);
        } else {
          ret = ">=".concat(M, ".").concat(m, ".").concat(p, "-").concat(pr, " <").concat(M, ".").concat(+m + 1, ".0");
        }
      } else {
        ret = ">=".concat(M, ".").concat(m, ".").concat(p, "-").concat(pr, " <").concat(+M + 1, ".0.0");
      }
    } else {
      debug('no pr');
      ...
      if (M === '0') {
        if (m === '0') {
          ret = ">=".concat(M, ".").concat(m, ".").concat(p, " <").concat(M, ".").concat(m, ".").concat(+p + 1);
        } else {
          ret = ">=".concat(M, ".").concat(m, ".").concat(p, " <").concat(M, ".").concat(+m + 1, ".0");
        }
      } else {
        ret = ">=".concat(M, ".").concat(m, ".").concat(p, " <").concat(+M + 1, ".0.0");
      }
    }
...
var replaceXRange = function replaceXRange(comp, options) {
  ...
  return comp.replace(r, function (ret, gtlt, M, m, p, pr) {
    ...
    if (xM) {
      ...
      ret = ">=".concat(M, ".0.0").concat(pr, " <").concat(+M + 1, ".0.0").concat(pr);
    } else if (xp) {
      ret = ">=".concat(M, ".").concat(m, ".0").concat(pr, " <").concat(M, ".").concat(+m + 1, ".0").concat(pr);
    }
...
var hyphenReplace = function hyphenReplace($0, from, fM, fm, fp, fpr, fb, to, tM, tm, tp, tpr, tb) {
  if (isX(fM)) {
    from = '';
  } else if (isX(fm)) {
    from = ">=".concat(fM, ".0.0");
  } else if (isX(fp)) {
    from = ">=".concat(fM, ".").concat(fm, ".0");
  } else {
    from = ">=".concat(from);
  }
