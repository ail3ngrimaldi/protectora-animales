...
var util = require("util"),
    ...
    _require = require("./source-code-utils"),
    getSourceCodeOfFiles = _require.getSourceCodeOfFiles;
...
function writeFile(config, format) {
  ...
  var installedESLint = config.installedESLint;
  delete config.installedESLint;
  ConfigFile.write(config, "./.eslintrc".concat(extname));
  log.info("Successfully created .eslintrc".concat(extname, " file in ").concat(process.cwd()));
...
function getPeerDependencies(moduleName) {
  var result = getPeerDependencies.cache.get(moduleName);

  if (!result) {
    log.info("Checking peerDependencies of ".concat(moduleName));
...
function getModulesList(config, installESLint) {
  var modules = {}; // Create a list of modules which should be installed based on config
  ...
  if (config.plugins) {
    ...
    try {
      for (var _iterator = config.plugins[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var plugin = _step.value;
        var moduleName = naming.normalizePackageName(plugin, "eslint-plugin");
  ...
  if (config["extends"]) {
    var extendList = Array.isArray(config["extends"]) ? config["extends"] : [config["extends"]];
    var _iteratorNormalCompletion2 = true;
    var _didIteratorError2 = false;
    var _iteratorError2 = undefined;

    try {
      for (var _iterator2 = extendList[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        var _moduleName = naming.normalizePackageName(extend, "eslint-config");

        modules[_moduleName] = "latest";
        Object.assign(modules, getPeerDependencies("".concat(_moduleName, "@latest")));
      }
    } catch (err) {
  ...
  var parser = config.parser || config.parserOptions && config.parserOptions.parser;
  ...
  if (installESLint === false) {
    ...
    var installStatus = npmUtils.checkDevDeps(["eslint"]); // Mark to show messages if it's new installation of eslint.
  ...
  return Object.keys(modules).map(function (name) {
    return "".concat(name, "@").concat(modules[name]);
  });
}
/**
...
function configureRules(answers, config) {
  var BAR_TOTAL = 20,
      ...
      disabledConfigs = {};
  var sourceCodes, registry; // Set up a progress bar, as this process can take a long time
  ...
  var patterns = answers.patterns.split(/[\t-\r \xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]+/);
  ...
  registry.populateFromCoreRules(); // Lint all files with each rule config in the registry

  registry = registry.lintSourceCode(sourceCodes, newConfig, function (total) {
    bar.tick((BAR_TOTAL - BAR_SOURCE_CODE_TOTAL) / total); // Subtract out ticks used at beginning
  });
  debug("\nRegistry: ".concat(util.inspect(registry.rules, {
    depth: null
  ...
  var specTwoConfigs = registry.filterBySpecificity(2).createConfig().rules; // Maybe a specific combination using all three options works

  var specThreeConfigs = registry.filterBySpecificity(3).createConfig().rules; // If all else fails, try to use the default (severity only)

  var defaultConfigs = registry.filterBySpecificity(1).createConfig().rules; // Combine configs in reverse priority order (later take precedence)

  newConfig.rules = Object.assign({}, disabledConfigs, defaultConfigs, specThreeConfigs, specTwoConfigs, singleConfigs); // Make sure progress bar has finished (floating point rounding)

  bar.update(BAR_TOTAL); // Log out some stats to let the user know what happened
  ...
  var resultMessage = ["\nEnabled ".concat(enabledRules, " out of ").concat(totalRules), "rules based on ".concat(fileQty), "file".concat(fileQty === 1 ? "." : "s.")].join(" ");
...
function getLocalESLintVersion() {
  try {
    var eslintPath = ModuleResolver.resolve("eslint", path.join(process.cwd(), "__placeholder__.js"));
...
function hasESLintVersionConflict(answers) {
  ...
  var localESLintVersion = getLocalESLintVersion();
  ...
  var configName = getStyleGuideName(answers);
  var moduleName = "eslint-config-".concat(configName, "@latest");
  var peerDependencies = getPeerDependencies(moduleName) || {};
  var requiredESLintVersionRange = peerDependencies.eslint;
  ...
  answers.requiredESLintVersionRange = requiredESLintVersionRange; // Check the version.
...
function installModules(modules) {
  log.info("Installing ".concat(modules.join(", ")));
...
function askInstallModules(modules, packageJsonExists) {
  ...
  return inquirer.prompt([{
    ...
    var executeInstallation = _ref.executeInstallation;
...
function promptUser() {
  return inquirer.prompt([{
    ...
    "default": "problems",
    choices: [{
      name: "To check syntax only",
      value: "syntax"
    }, {
      name: "To check syntax and find problems",
      value: "problems"
    }, {
      name: "To check syntax, find problems, and enforce code style",
      value: "style"
    }]
  }, {
    type: "list",
    ...
    "default": "guide",
    choices: [{
      name: "Use a popular style guide",
      value: "guide"
    }, {
      name: "Answer questions about your style",
      value: "prompt"
    }, {
      name: "Inspect your JavaScript file(s)",
      value: "auto"
    }],
    ...
    message: function message(answers) {
      var verb = semver.ltr(answers.localESLintVersion, answers.requiredESLintVersionRange) ? "upgrade" : "downgrade";
      return "The style guide \"".concat(answers.styleguide, "\" requires eslint@").concat(answers.requiredESLintVersionRange, ". You are currently using eslint@").concat(answers.localESLintVersion, ".\n  Do you want to ").concat(verb, "?");
    },
    "default": true,
    when: function when(answers) {
      return answers.source === "guide" && answers.packageJsonExists && hasESLintVersionConflict(answers);
    }
  }]).then(function (earlyAnswers) {
    // early exit if no style guide is necessary
    if (earlyAnswers.purpose !== "style") {
      var config = processAnswers(earlyAnswers);
      var modules = getModulesList(config);
      return askInstallModules(modules, earlyAnswers.packageJsonExists).then(function () {
        return writeFile(config, earlyAnswers.format);
      });
    ...
    if (earlyAnswers.source === "guide") {
      ...
      if (earlyAnswers.installESLint === false && !semver.satisfies(earlyAnswers.localESLintVersion, earlyAnswers.requiredESLintVersionRange)) {
        log.info("Note: it might not work since ESLint's version is mismatched with the ".concat(earlyAnswers.styleguide, " config."));
      }
      ...
      var _config = processAnswers(earlyAnswers);

      if (Array.isArray(_config["extends"])) {
        _config["extends"].push(earlyAnswers.styleguide);
      } else if (_config["extends"]) {
        _config["extends"] = [_config["extends"], earlyAnswers.styleguide];
      } else {
        _config["extends"] = [earlyAnswers.styleguide];
      }

      var _modules = getModulesList(_config);

      return askInstallModules(_modules, earlyAnswers.packageJsonExists).then(function () {
        return writeFile(_config, earlyAnswers.format);
      });
    }

    if (earlyAnswers.source === "auto") {
      var combinedAnswers = Object.assign({}, earlyAnswers);

      var _config2 = processAnswers(combinedAnswers);

      var _modules2 = getModulesList(_config2);

      return askInstallModules(_modules2).then(function () {
        return writeFile(_config2, earlyAnswers.format);
      });
    ...
    return inquirer.prompt([{
      ...
      var totalAnswers = Object.assign({}, earlyAnswers, answers);
      var config = processAnswers(totalAnswers);
      var modules = getModulesList(config);
      return askInstallModules(modules).then(function () {
        return writeFile(config, earlyAnswers.format);
      });
    });
  });
