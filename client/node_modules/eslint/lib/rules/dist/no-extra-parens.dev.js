...
var _require = require("eslint-utils"),
    isParenthesizedRaw = _require.isParenthesized;
...
module.exports = {
  ...
  create: function create(context) {
    ...
    var IGNORE_SEQUENCE_EXPRESSIONS = ALL_NODES && context.options[1] && context.options[1].enforceForSequenceExpressions === false;
    var IGNORE_NEW_IN_MEMBER_EXPR = ALL_NODES && context.options[1] && context.options[1].enforceForNewInMemberExpressions === false;
    var PRECEDENCE_OF_ASSIGNMENT_EXPR = precedence({
      type: "AssignmentExpression"
    });
    var PRECEDENCE_OF_UPDATE_EXPR = precedence({
    ...
    function requiresTrailingSpace(node) {
      var nextTwoTokens = sourceCode.getTokensAfter(node, {
        count: 2
      });
    ...
    function checkExpressionOrExportStatement(node) {
      ...
      var tokenAfterClosingParens = secondToken ? sourceCode.getTokenAfter(secondToken, astUtils.isNotClosingParenToken) : null;
    ...
    function endCurrentReportsBuffering() {
      var _reportsBuffer = reportsBuffer,
          upper = _reportsBuffer.upper,
          inExpressionNodes = _reportsBuffer.inExpressionNodes,
          reports = _reportsBuffer.reports;

      if (upper) {
        var _upper$inExpressionNo, _upper$reports;

        (_upper$inExpressionNo = upper.inExpressionNodes).push.apply(_upper$inExpressionNo, _toConsumableArray(inExpressionNodes));

        (_upper$reports = upper.reports).push.apply(_upper$reports, _toConsumableArray(reports));
      } else {
        // flush remaining reports
        reports.forEach(function (_ref) {
          var finishReport = _ref.finishReport;
          return finishReport();
        });
      }
    ...
    function isInCurrentReportsBuffer(node) {
      return reportsBuffer.reports.some(function (r) {
        return r.node === node;
      });
    }
    /**
    ...
    function removeFromCurrentReportsBuffer(node) {
      reportsBuffer.reports = reportsBuffer.reports.filter(function (r) {
        return r.node !== node;
      });
    }
    ...
    return {
      ...
      ClassBody: function ClassBody(node) {
        node.body.filter(function (member) {
          return member.type === "MethodDefinition" && member.computed && member.key && hasExcessParens(member.key) && precedence(member.key) >= PRECEDENCE_OF_ASSIGNMENT_EXPR;
        }).forEach(function (member) {
          return report(member.key);
        });
      },
      ...
      "ForStatement > *.init:exit": function ForStatementInitExit(node) {
        /*
        ...
        if (reportsBuffer.reports.length) {
          reportsBuffer.inExpressionNodes.forEach(function (inExpressionNode) {
            var path = pathToDescendant(node, inExpressionNode);
            var nodeToExclude;
