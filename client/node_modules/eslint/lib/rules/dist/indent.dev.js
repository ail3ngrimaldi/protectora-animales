...
function () {
  ...
  _createClass(OffsetStorage, [{
    ...
    value: function setDesiredOffsets(range, fromToken, offset, force) {
      ...
      var fromTokenDescriptor = fromTokenIsInRange && this._getOffsetDescriptor(fromToken); // First, remove any existing nodes in the range from the tree.
    ...
    value: function getDesiredIndent(token) {
      if (!this._desiredIndentCache.has(token)) {
        if (this._ignoredTokens.has(token)) {
          ...
          var offset = offsetInfo.from && offsetInfo.from.loc.start.line === token.loc.start.line && !/^[\t-\r \xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]*?\n/.test(token.value) && !offsetInfo.force ? 0 : offsetInfo.offset * this._indentSize;
...
module.exports = {
  ...
  create: function create(context) {
    ...
    function createErrorMessageData(expectedAmount, actualSpaces, actualTabs) {
      var expectedStatement = "".concat(expectedAmount, " ").concat(indentType).concat(expectedAmount === 1 ? "" : "s"); // e.g. "2 tabs"

      var foundSpacesWord = "space".concat(actualSpaces === 1 ? "" : "s"); // e.g. "space"

      var foundTabsWord = "tab".concat(actualTabs === 1 ? "" : "s"); // e.g. "tabs"
    ...
    function report(token, neededIndent) {
      ...
      var numSpaces = actualIndent.filter(function (_char) {
        return _char === " ";
      }).length;
      var numTabs = actualIndent.filter(function (_char2) {
        return _char2 === "\t";
      }).length;
      context.report({
        node: token,
    ...
    function countTrailingLinebreaks(string) {
      var trailingWhitespace = string.match(/[\t-\r \xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]*$/)[0];
      var linebreakMatches = trailingWhitespace.match(astUtils.createGlobalLinebreakMatcher());
    ...
    function addParensIndent(tokens) {
      ...
      parenPairs.forEach(function (pair) {
        ...
        var rightParen = pair.right; // We only want to handle parens around expressions, so exclude parentheses that are in function parameters and function call arguments.
    ...
    function ignoreNode(node) {
      var unknownNodeTokens = new Set(sourceCode.getTokens(node, {
    ...
    var baseOffsetListeners = {
      ...
      ConditionalExpression: function ConditionalExpression(node) {
        var firstToken = sourceCode.getFirstToken(node); // `flatTernaryExpressions` option is for the following style:
        // var a =
        //     foo > 0 ? bar :
        //     foo < 0 ? baz :
        //     /*else*/ qiz ;
      ...
      ExportNamedDeclaration: function ExportNamedDeclaration(node) {
        if (node.declaration === null) {
          var closingCurly = sourceCode.getLastToken(node, astUtils.isClosingBraceToken); // Indent the specifiers in `export {foo, bar, baz}`
      ...
      ImportDeclaration: function ImportDeclaration(node) {
        ...
        var sourceToken = sourceCode.getLastToken(node, function (token) {
          return token.type === "String";
        });
        var semiToken = sourceCode.getLastToken(node, function (token) {
          return token.type === "Punctuator" && token.value === ";";
        });

        if (fromToken) {
          var end = semiToken && semiToken.range[1] === sourceToken.range[1] ? node.range[1] : sourceToken.range[1];
          offsets.setDesiredOffsets([fromToken.range[0], end], sourceCode.getFirstToken(node), 1);
        }
      },
      ImportExpression: function ImportExpression(node) {
        var openingParen = sourceCode.getFirstToken(node, 1);
        var closingParen = sourceCode.getLastToken(node);
        parameterParens.add(openingParen);
        parameterParens.add(closingParen);
        offsets.setDesiredOffset(openingParen, sourceCode.getTokenBefore(openingParen), 0);
      ...
      "MemberExpression, JSXMemberExpression, MetaProperty": function MemberExpressionJSXMemberExpressionMetaProperty(node) {
        ...
        var objectParenCount = sourceCode.getTokensBetween(object, node.property, {
          filter: astUtils.isClosingParenToken
        }).length;
        var firstObjectToken = objectParenCount ? sourceCode.getTokenBefore(object, {
      ...
      SwitchCase: function SwitchCase(node) {
        if (!(node.consequent.length === 1 && node.consequent[0].type === "BlockStatement")) {
          var caseKeyword = sourceCode.getFirstToken(node);
          var tokenAfterCurrentCase = sourceCode.getTokenAfter(node);
      ...
      JSXFragment: function JSXFragment(node) {
        var firstOpeningToken = sourceCode.getFirstToken(node.openingFragment);
        var firstClosingToken = sourceCode.getFirstToken(node.closingFragment);
        addElementListIndent(node.children, firstOpeningToken, firstClosingToken, 1);
      },
      JSXOpeningFragment: function JSXOpeningFragment(node) {
        var firstToken = sourceCode.getFirstToken(node);
        var closingToken = sourceCode.getLastToken(node);
        offsets.setDesiredOffsets(node.range, firstToken, 1);
        offsets.matchOffsetOf(firstToken, closingToken);
      },
      JSXClosingFragment: function JSXClosingFragment(node) {
        var firstToken = sourceCode.getFirstToken(node);
        var slashToken = sourceCode.getLastToken(node, {
          skip: 1
        });
        var closingToken = sourceCode.getLastToken(node);
        var tokenToMatch = astUtils.isTokenOnSameLine(slashToken, closingToken) ? slashToken : closingToken;
        offsets.setDesiredOffsets(node.range, firstToken, 1);
      ...
      "*": function _(node) {
        var firstToken = sourceCode.getFirstToken(node); // Ensure that the children of every node are indented at least as much as the first token.
    ...
    return Object.assign(offsetListeners, ignoredNodeListeners, {
      ...
      "Program:exit": function ProgramExit() {
        ...
        var precedingTokens = sourceCode.ast.comments.reduce(function (commentMap, comment) {
          var tokenOrCommentBefore = sourceCode.getTokenBefore(comment, {
            includeComments: true
          });
