...
module.exports = {
  ...
  create: function create(context) {
    ...
    function isConstructor(name) {
      var match = CTOR_PREFIX_REGEX.exec(name); // Not a constructor if name has no characters apart from '_', '$' and digits e.g. '_', '$$', '_8'

      if (!match) {
        return false;
      }

      var firstChar = name.charAt(match.index);
    ...
    function makeFunctionShorthand(fixer, node) {
      ...
      var fixRange = [firstKeyToken.range[0], node.range[1]];
      var methodPrefix = keyPrefix + keyText;
      ...
      if (node.value.type === "FunctionExpression") {
        ...
        var _tokenBeforeParams = node.value.generator ? sourceCode.getTokenAfter(functionToken) : functionToken;

        return fixer.replaceTextRange(fixRange, methodPrefix + sourceCode.text.slice(_tokenBeforeParams.range[1], node.value.range[1]));
      }

      var arrowToken = sourceCode.getTokenBefore(node.value.body, astUtils.isArrowToken);
      var fnBody = sourceCode.text.slice(arrowToken.range[1], node.value.range[1]);
      var shouldAddParensAroundParameters = false;
      var tokenBeforeParams;
      ...
      if (node.value.params.length === 1) {
        var hasParen = astUtils.isOpeningParenToken(tokenBeforeParams);
        var isTokenOutsideNode = tokenBeforeParams.range[0] < node.range[0];
        shouldAddParensAroundParameters = !hasParen || isTokenOutsideNode;
      }

      var sliceStart = shouldAddParensAroundParameters ? node.value.params[0].range[0] : tokenBeforeParams.range[0];
      var sliceEnd = sourceCode.getTokenBefore(arrowToken).range[1];
      var oldParamText = sourceCode.text.slice(sliceStart, sliceEnd);
      var newParamText = shouldAddParensAroundParameters ? "(".concat(oldParamText, ")") : oldParamText;
    ...
    return {
      ...
      "Property:exit": function PropertyExit(node) {
        var isConciseProperty = node.method || node.shorthand; // Ignore destructuring assignment
        ...
        if (isConciseProperty) {
          if (node.method && (APPLY_NEVER || AVOID_QUOTES && isStringLiteral(node.key))) {
            var message = APPLY_NEVER ? "Expected longform method syntax." : "Expected longform method syntax for string literal keys."; // { x() {} } should be written as { x: function() {} }
            ...
            context.report({
              ...
              fix: function fix(fixer) {
                return fixer.insertTextAfter(node.key, ": ".concat(node.key.name));
              }
            });
          }
