...
var buf2 = Buffer.allocUnsafe(SIZE); // Default encoding
...
testBufs("a\u0234b\u0235c\u0236", 4, 1, 'utf8');
testBufs("a\u0234b\u0235c\u0236", 12, 1, 'utf8');
assert.equal(Buffer.allocUnsafe(1).fill(0).fill("\u0222")[0], 0xc8); // BINARY
...
testBufs("a\u0234b\u0235c\u0236", 4, 1, 'binary');
testBufs("a\u0234b\u0235c\u0236", 12, 1, 'binary'); // LATIN1
...
testBufs("a\u0234b\u0235c\u0236", 4, 1, 'latin1');
testBufs("a\u0234b\u0235c\u0236", 12, 1, 'latin1'); // UCS2
...
assert.equal(Buffer.allocUnsafe(1).fill("\u0222", 'ucs2')[0], os.endianness() === 'LE' ? 0x22 : 0x02); // HEX
...
testBufs('61c8b462c8b563c8b6', 4, 1, 'hex');
testBufs('61c8b462c8b563c8b6', 12, 1, 'hex'); // Make sure this operation doesn't go on forever
...
testBufs('Yci0Ysi1Y8i2', 4, 1, 'ucs2');
testBufs('Yci0Ysi1Y8i2', 12, 1, 'ucs2'); // Buffer
...
deepStrictEqualValues(genBuffer(4, [hexBufFill, 1, -1]), [0, 0, 0, 0]); // Check exceptions
...
function writeToFill(string, offset, end, encoding) {
  ...
  assert(offset <= buf2.length); // Convert "end" to "length" (which write understands).
  ...
  do {
    ...
    offset += written; // Safety check in case write falls into infinite loop.

    if (written === 0) {
      if (wasZero) throw new Error('Could not write all data to Buffer');else wasZero = true;
    }
  } while (offset < buf2.length); // Correction for UCS2 operations.
...
function testBufs(string, offset, length, encoding) {
  bufReset();
  buf1.fill.apply(buf1, arguments); // Swap bytes on BE archs for ucs2 encoding.
