...
assert.equal(b.indexOf(0x0), -1); // test offsets
...
assert.equal(Buffer.from('ff').indexOf(Buffer.from('f'), 1, 'ucs2'), -1); // test hex encoding

assert.strictEqual(Buffer.from(b.toString('hex'), 'hex').indexOf('64', 0, 'hex'), 3);
assert.strictEqual(Buffer.from(b.toString('hex'), 'hex').indexOf(Buffer.from('64', 'hex'), 0, 'hex'), 3); // test base64 encoding

assert.strictEqual(Buffer.from(b.toString('base64'), 'base64').indexOf('ZA==', 0, 'base64'), 3);
assert.strictEqual(Buffer.from(b.toString('base64'), 'base64').indexOf(Buffer.from('ZA==', 'base64'), 0, 'base64'), 3); // test ascii encoding

assert.strictEqual(Buffer.from(b.toString('ascii'), 'ascii').indexOf('d', 0, 'ascii'), 3);
assert.strictEqual(Buffer.from(b.toString('ascii'), 'ascii').indexOf(Buffer.from('d', 'ascii'), 0, 'ascii'), 3); // test latin1 encoding
...
assert.strictEqual(Buffer.from("\xE8", 'latin1').indexOf(Buffer.from("\xE8", 'latin1'), 'latin1'), 0); // test binary encoding
...
assert.strictEqual(Buffer.from("\xE8", 'binary').indexOf(Buffer.from("\xE8", 'binary'), 'binary'), 0); // test optional offset with passed encoding
...
{
  var twoByteString = Buffer.from("\u039A\u0391\u03A3\u03A3\u0395", 'ucs2'); // Test single char pattern
  ...
  assert.equal(-1, twoByteString.indexOf("\u0392", 0, 'ucs2'), 'Not beta'); // Test multi-char pattern
...
assert.equal(-1, mixedByteStringUtf8.indexOf("\u0396")); // Test complex string indexOf algorithms. Only trigger for long strings.
...
assert.equal(1535, longBufferString.indexOf(pattern, 512), 'Long JABACABA..., Second J'); // Search for a non-ASCII string in a pure ASCII string.
...
assert.equal(3, asciiString.indexOf('leb', 0)); // Search in string containing many non-ASCII chars.
...
assert.equal(-1, allCharsBufferUcs2.indexOf('notfound')); // Needle is longer than haystack, but only because it's encoded as UTF-16
...
assert.strictEqual(Buffer.from('aaaa').indexOf('你好', 'ucs2'), -1); // Haystack has odd length, but the needle is UCS2.
...
assert.equal(b.lastIndexOf('a', Infinity), 0); // lastIndexOf Buffer:
...
assert.equal(b.lastIndexOf(buf_empty, Infinity), -1); // lastIndexOf number:
...
assert.equal(b.lastIndexOf(0x0), -1); // Test weird offset arguments.
...
assert.equal(b.lastIndexOf('b', []), -1);
assert.equal(b.lastIndexOf('b', [2]), 1); // Test needles longer than the haystack.
...
assert.strictEqual(b.lastIndexOf(Buffer.from('aaaaaaaaaaaaaaa'), 7), -1); // 你好 expands to a total of 6 bytes using UTF-8 and 4 bytes using UTF-16
...
assert.strictEqual(buf_bc.lastIndexOf(Buffer.from('你好'), 7), -1); // Test lastIndexOf on a longer buffer:
...
assert.equal(-1, bufferString.lastIndexOf('a ', -28)); // Test lastIndexOf for the case that the first character can be found,
...
assert.strictEqual(0, Buffer.from('abc').lastIndexOf('ab', 3)); // The above tests test the LINEAR and SINGLE-CHAR strategies.
...
assert.equal(511, longBufferString.lastIndexOf(pattern, 1534)); // Finally, give it a really long input to trigger fallback from BMH to
...
assert.equal('yolo swag swag yolo', reallyLong.slice(0, 19).toString()); // Expensive reverse searches. Stress test lastIndexOf:
...
assert.equal(0, reallyLong.lastIndexOf(pattern)); // test truncation of Number arguments to uint8
