...
var Buffer = require('../../').Buffer; // counter to ensure unique value is always copied


var cntr = 0;
var b = Buffer.allocUnsafe(1024); // console.log('b.length == %d', b.length);
...
var c = Buffer.allocUnsafe(512); // console.log('c.length == %d', c.length);
...
}
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = b.copy(c, 0, 0, 512); //   console.log('copied %d bytes from b into c', copied);
...
}
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = c.copy(b, 0, 0); //   console.log('copied %d bytes from c into b w/o sourceEnd', copied);
...
}
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = c.copy(b, 0); //   console.log('copied %d bytes from c into b w/o sourceStart', copied);
...
}
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = b.copy(c); //   console.log('copied %d bytes from b into c w/o targetStart', copied);
...
}
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = b.copy(c, 0, b.length - Math.floor(c.length / 2)); //   console.log('copied %d bytes from end of b into beginning of c', copied);
...
}
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = b.copy(c, 0, 0, 513); //   console.log('copied %d bytes from b trying to overrun c', copied);
...
}
{
  ...
  b.fill(++cntr);
  b.fill(++cntr, 256);
  var copied = b.copy(b, 0, 256, 1024); //   console.log('copied %d bytes from b into b', copied);
...
bb.fill('hello crazy world'); // try to copy from before the beginning of b
...
}, RangeError);
{
  ...
  b.fill(++cntr);
  c.fill(++cntr);
  var copied = b.copy(c, 0, 0, 1025); //   console.log('copied %d bytes from b into c', copied);
...
assert["throws"](function () {
  b.copy(c, 0, 0, -1);
}, RangeError); // when sourceStart is greater than sourceEnd, zero copied

assert.equal(b.copy(c, 0, 100, 10), 0); // when targetStart > targetLength, zero copied

assert.equal(b.copy(c, 512, 0, 10), 0);
var caught_error; // invalid encoding for Buffer.toString
...
assert.strictEqual('Unknown encoding: invalid', caught_error.message); // invalid encoding for Buffer.write
...
assert.strictEqual('Unknown encoding: invalid', caught_error.message); // try to create 0-length buffers

Buffer.from('');
Buffer.from('', 'ascii');
Buffer.from('', 'latin1');
Buffer.alloc(0);
Buffer.allocUnsafe(0); // try to write a 0-length string beyond the end of b

assert["throws"](function () {
  b.write('', 2048);
}, RangeError); // throw when writing to negative offset

assert["throws"](function () {
  b.write('a', -1);
}, RangeError); // throw when writing past bounds from the pool
...
assert["throws"](function () {
  b.write('a', -1);
}, RangeError); // try to copy 0 bytes worth of data into an empty buffer

b.copy(Buffer.alloc(0), 0, 0, 0); // try to copy 0 bytes past the end of the target buffer

b.copy(Buffer.alloc(0), 1, 1, 1);
b.copy(Buffer.alloc(1), 1, 1, 1); // try to copy 0 bytes from past the end of the source buffer

b.copy(Buffer.alloc(1), 0, 2048, 2048);
var rangeBuffer = Buffer.from('abc'); // if start >= buffer's length, empty string will be returned
...
assert.equal(rangeBuffer.toString('ascii', 'Infinity', 3), ''); // if end <= 0, empty string will be returned
...
assert.equal(rangeBuffer.toString('ascii', 1, -Infinity), ''); // if start < 0, start will be taken as zero
...
assert.equal(rangeBuffer.toString('ascii', '-1', 3), 'abc');
assert.equal(rangeBuffer.toString('ascii', '-1.99', 3), 'abc');
assert.equal(rangeBuffer.toString('ascii', '-Infinity', 3), 'abc'); // if start is an invalid integer, start will be taken as zero
...
assert.equal(rangeBuffer.toString('ascii', '', 3), 'abc'); // but, if start is an integer when coerced, then it will be coerced and used.
...
assert.equal(rangeBuffer.toString('ascii', '-1.99', 3), 'abc');
assert.equal(rangeBuffer.toString('ascii', 1.99, 3), 'bc');
assert.equal(rangeBuffer.toString('ascii', true, 3), 'bc'); // if end > buffer's length, end will be taken as buffer's length
...
assert.equal(rangeBuffer.toString('ascii', 0, '5'), 'abc');
assert.equal(rangeBuffer.toString('ascii', 0, '6.99'), 'abc');
assert.equal(rangeBuffer.toString('ascii', 0, 'Infinity'), 'abc'); // if end is an invalid integer, end will be taken as buffer's length
...
assert.equal(rangeBuffer.toString('ascii', 0, ''), ''); // but, if end is an integer when coerced, then it will be coerced and used.
...
assert.equal(rangeBuffer.toString('ascii', 0, 1.99), 'a');
assert.equal(rangeBuffer.toString('ascii', 0, true), 'a'); // try toString() with a object as a encoding

assert.equal(rangeBuffer.toString({
  toString: function toString() {
    return 'ascii';
  }
}), 'abc'); // testing for smart defaults and ability to pass string values as offset
...
assert.equal(writeTest.toString(), 'nodejs'); // ASCII slice test
...
{
  // Bug regression test
  var testValue = "\xF6\u65E5\u672C\u8A9E"; // ö日本語

  var buffer = Buffer.allocUnsafe(32);
  var size = buffer.write(testValue, 0, 'utf8'); //   console.log('bytes written to buffer: ' + size);
...
{
  var e = Buffer.from('über'); //   console.error('uber: \'%s\'', e.toString());

  assert.deepStrictEqual(e, Buffer.from([195, 188, 98, 101, 114]));
}
{
  var f = Buffer.from('über', 'ascii'); //   console.error('f.length: %d     (should be 4)', f.length);

  assert.deepStrictEqual(f, Buffer.from([252, 98, 101, 114]));
}
['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function (encoding) {
  {
    var f = Buffer.from('über', encoding); //     console.error('f.length: %d     (should be 8)', f.length);

    assert.deepStrictEqual(f, Buffer.from([252, 0, 98, 0, 101, 0, 114, 0]));
  }
  {
    var f = Buffer.from('привет', encoding); //     console.error('f.length: %d     (should be 12)', f.length);
  ...
  }
  {
    ...
    var size = f.write('あいうえお', encoding); //     console.error('bytes written to buffer: %d     (should be 4)', size);
...
}
{
  ...
  assert.equal(quote, b.toString('ascii', 0, quote.length)); // check that the base64 decoder ignores whitespace

  var expectedWhite = expected.slice(0, 60) + ' \n' + expected.slice(60, 120) + ' \n' + expected.slice(120, 180) + ' \n' + expected.slice(180, 240) + ' \n' + expected.slice(240, 300) + '\n' + expected.slice(300, 360) + '\n';
  b = Buffer.allocUnsafe(1024);
  bytesWritten = b.write(expectedWhite, 0, 'base64');
  assert.equal(quote.length, bytesWritten);
  assert.equal(quote, b.toString('ascii', 0, quote.length)); // check that the base64 decoder on the constructor works
  // even in the presence of whitespace.

  b = Buffer.from(expectedWhite, 'base64');
  assert.equal(quote.length, b.length);
  assert.equal(quote, b.toString('ascii', 0, quote.length)); // check that the base64 decoder ignores illegal chars
...
assert.equal(Buffer.from('K', 'base64').toString(), ''); // multiple-of-4 with padding
...
assert.equal(Buffer.from('KioqKioqKioqKioqKioqKioqKio=', 'base64').toString(), '********************'); // no padding, not a multiple of 4
...
assert.equal(Buffer.from('KioqKioqKioqKioqKioqKio', 'base64').toString(), '*****************');
assert.equal(Buffer.from('KioqKioqKioqKioqKioqKioqKg', 'base64').toString(), '*******************');
assert.equal(Buffer.from('KioqKioqKioqKioqKioqKioqKio', 'base64').toString(), '********************'); // handle padding graciously, multiple-of-4 or not
...
assert.equal(Buffer.from('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg=', 'base64').length, 31);
assert.equal(Buffer.from('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg', 'base64').length, 31); // This string encodes single '.' character in UTF-16
...
assert.equal(0, Buffer.from('hello').slice(0, 0).length); // test hex toString
...
var x = buildBuffer([0x81, 0xa3, 0x66, 0x6f, 0x6f, 0xa3, 0x62, 0x61, 0x72]); // console.log(x.inspect());

assert.equal('<Buffer 81 a3 66 6f 6f a3 62 61 72>', x.inspect());
{
  var z = x.slice(4); //   console.log(z.inspect());
...
{
  var z = x.slice(0); //   console.log(z.inspect());
  //   console.log(z.length);

  assert.equal(z.length, x.length);
}
{
  var z = x.slice(0, 4); //   console.log(z.inspect());
  //   console.log(z.length);
...
{
  var z = x.slice(0, 9); //   console.log(z.inspect());
  //   console.log(z.length);

  assert.equal(9, z.length);
}
{
  var z = x.slice(1, 4); //   console.log(z.inspect());
  //   console.log(z.length);
...
{
  var z = x.slice(2, 4); //   console.log(z.inspect());
  //   console.log(z.length);
...
}
{
  ...
  buf.fill(0xFF);
  var written = buf.write('abcd', 1, 2, 'utf8'); //   console.log(buf);
  ...
  buf.fill(0xFF);
  written = buf.write('abcd', 1, 4); //   console.log(buf);
  ...
  buf.fill(0xFF);
  written = buf.write('abcd', 1, 2, 'utf8'); //   console.log(buf);
  ...
  assert.equal(buf[3], 0xFF);
  buf.fill(0xFF);
  written = buf.write('abcdef', 1, 2, 'hex'); //   console.log(buf);
  ...
  ['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function (encoding) {
    buf.fill(0xFF);
    written = buf.write('abcd', 0, 2, encoding); //     console.log(buf);
...
Buffer.allocUnsafe(3.3).fill().toString(); // throws bad argument error in commit 43cb4ec
...
assert.equal(Buffer.from({
  length: 'BAM'
}).length, 0); // Make sure that strings are not coerced to numbers.

assert.equal(Buffer.from('99').length, 2);
assert.equal(Buffer.from('13.37').length, 5); // Ensure that the length argument is respected.
...
}, RangeError); // attempt to overflow buffers, similar to previous bug in array buffers
...
assert["throws"](function () {
  var buf = Buffer.allocUnsafe(8);
  buf.writeFloatLE(0.0, 0xffffffff);
}, RangeError); // ensure negative values can't get past offset
...

{
  ...
  var utf16Buf = Buffer.from('0123456789', 'utf16le'); // assert.deepStrictEqual(utf16Buf.slice(0, 6), Buffer.from('012', 'utf16le'));
  ...
  assert.equal(buf.slice('0', '-111'), ''); // try to slice a zero length Buffer
...
assert["throws"](function () {
  Buffer.from('', 'buffer');
}, TypeError); // Regression test for #6111. Constructing a buffer from another buffer
...
Buffer.poolSize = ps; // Test Buffer.copy() segfault
...
assert["throws"](function () {
  Buffer.from(null);
}, regErrorMsg); // Test that ParseArrayIndex handles full uint32

assert["throws"](function () {
  Buffer.from(new ArrayBuffer(0), -1 >>> 0);
}, /RangeError: 'offset' is out of bounds/); // ParseArrayIndex() should reject values that don't fit in a 32 bits size_t.

assert["throws"](function () {
  var a = Buffer(1).fill(0);
  var b = Buffer(1).fill(0);
  a.copy(b, 0, 0x100000000, 0x100000001);
}), /out of range index/; // Unpooled buffer (replaces SlowBuffer)

var ubuf = Buffer.allocUnsafeSlow(10);
assert(ubuf);
assert(ubuf.buffer);
assert.equal(ubuf.buffer.byteLength, 10); // Regression test
