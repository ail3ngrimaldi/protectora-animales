...
var SlowBuffer = require('../../').SlowBuffer; // counter to ensure unique value is always copied
...
var c = Buffer(512); // console.log('c.length == %d', c.length);
...
new Buffer('', 'latin1');
new Buffer('', 'binary');
Buffer(0); // try to write a 0-length string beyond the end of b
...
b.copy(Buffer(0), 0, 0, 0); // try to copy 0 bytes past the end of the target buffer

b.copy(Buffer(0), 1, 1, 1);
b.copy(Buffer(1), 1, 1, 1); // try to copy 0 bytes from past the end of the source buffer

b.copy(Buffer(1), 0, 2048, 2048);
var rangeBuffer = new Buffer('abc'); // if start >= buffer's length, empty string will be returned
...
{
  var e = new Buffer('über'); //   console.error('uber: \'%s\'', e.toString());

  assert.deepStrictEqual(e, new Buffer([195, 188, 98, 101, 114]));
}
{
  var f = new Buffer('über', 'ascii'); //   console.error('f.length: %d     (should be 4)', f.length);

  assert.deepStrictEqual(f, new Buffer([252, 98, 101, 114]));
}
['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function (encoding) {
  {
    var f = new Buffer('über', encoding); //     console.error('f.length: %d     (should be 8)', f.length);

    assert.deepStrictEqual(f, new Buffer([252, 0, 98, 0, 101, 0, 114, 0]));
  }
  {
    var f = new Buffer('привет', encoding); //     console.error('f.length: %d     (should be 12)', f.length);
...
assert.equal(new Buffer('K', 'base64').toString(), ''); // multiple-of-4 with padding
...
assert.equal(new Buffer('KioqKioqKioqKioqKioqKioqKio=', 'base64').toString(), '********************'); // no padding, not a multiple of 4
...
assert.equal(new Buffer('KioqKioqKioqKioqKioqKio', 'base64').toString(), '*****************');
assert.equal(new Buffer('KioqKioqKioqKioqKioqKioqKg', 'base64').toString(), '*******************');
assert.equal(new Buffer('KioqKioqKioqKioqKioqKioqKio', 'base64').toString(), '********************'); // handle padding graciously, multiple-of-4 or not
...
assert.equal(new Buffer('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg=', 'base64').length, 31);
assert.equal(new Buffer('w69jACy6BgZmaFvv96HG6MYksWytuZu3T1FvGnulPg', 'base64').length, 31); // This string encodes single '.' character in UTF-16
...
assert.equal(0, Buffer('hello').slice(0, 0).length); // test hex toString
...
assert["throws"](function () {
  Buffer.from('A', 'hex');
}, TypeError); // Test single base64 char encodes as 0
...
assert.equal(Buffer('13.37').length, 5); // Ensure that the length argument is respected.
...
assert["throws"](function () {
  new Buffer(null);
}, regErrorMsg); // Test prototype getters don't throw
...
assert.equal(SlowBuffer.prototype.offset, undefined);
{
  ...
  assert.deepStrictEqual(Buffer({
    length: -Buffer.poolSize
  }), Buffer.from(''));
  assert.deepStrictEqual(Buffer({
    length: -100
  }), Buffer.from('')); // Check pool offset after that by trying to write string into the pool.
...

{
  ...
  for (var _i = 0, _allocators = allocators; _i < _allocators.length; _i++) {
    var allocator = _allocators[_i];
