...
cp.execSync('rm -rf node/*.js', {
  cwd: path.join(__dirname, '../test')
});
var httpOpts = {
  headers: {
    'User-Agent': null // auth if github rate-limits you...
...
function testfixer(filename) {
  ...
  return through(function (line, enc, cb) {
    ...
    line = line.replace(/(const|let) /g, 'var '); // make `var common = require('common')` work

    line = line.replace(/(var common = require.*)/g, 'var common = { skip: function () {} };'); // make `require('../common')` work

    line = line.replace(/require\('\.\.\/common'\);/g, ''); // require browser buffer

    line = line.replace(/(.*)require\('buffer'\)(.*)/g, '$1require(\'../../\')$2'); // comment out console logs

    line = line.replace(/(.*console\..*)/g, '// $1'); // we can't reliably test typed array max-sizes in the browser
