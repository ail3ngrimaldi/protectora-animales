...
test('node 6 buffer indexOf tests', function (t) {
  ...
  t.equal(bindexOf(b, 0x0), -1); // test offsets
  ...
  t.equal(bindexOf(b, new Buffer('f'), 6), -1); // This one doesn't make any sense
  // t.equal(bindexOf(new Buffer('ff'), new Buffer('f'), 1, 'ucs2'), -1);
  // test hex encoding

  t.equal(bindexOf(new Buffer(b.toString('hex'), 'hex'), '64', 0, 'hex'), 3);
  t.equal(bindexOf(new Buffer(b.toString('hex'), 'hex'), new Buffer('64', 'hex'), 0, 'hex'), 3); // test base64 encoding

  t.equal(bindexOf(new Buffer(b.toString('base64'), 'base64'), 'ZA==', 0, 'base64'), 3);
  t.equal(bindexOf(new Buffer(b.toString('base64'), 'base64'), new Buffer('ZA==', 'base64'), 0, 'base64'), 3); // test ascii encoding

  t.equal(bindexOf(new Buffer(b.toString('ascii'), 'ascii'), 'd', 0, 'ascii'), 3);
  t.equal(bindexOf(new Buffer(b.toString('ascii'), 'ascii'), new Buffer('d', 'ascii'), 0, 'ascii'), 3); // test latin1 encoding
  ...
  */
  ...
  t.equal(bindexOf(new Buffer(b.toString('binary'), 'binary'), new Buffer('d', 'binary'), 0, 'binary'), 3);
  t.equal(bindexOf(new Buffer("aa\xE8aa", 'binary'), "\xE8", 0, 'binary'), 2);
  t.equal(bindexOf(new Buffer("\xE8", 'binary'), "\xE8", 0, 'binary'), 0);
  t.equal(bindexOf(new Buffer("\xE8", 'binary'), new Buffer("\xE8", 'binary'), 0, 'binary'), 0); // test optional offset with passed encoding
  ...
  {
    var twoByteString = new Buffer("\u039A\u0391\u03A3\u03A3\u0395", 'ucs2'); // Test single char pattern
    ...
    t.equal(-1, twoByteString.indexOf("\u0392", 0, 'ucs2'), 'Not beta'); // Test multi-char pattern
  ...
  var longBufferString = new Buffer(longString); // pattern of 15 chars, repeated every 16 chars in long
  ...
  t.equal(1535, longBufferString.indexOf(pattern, 512), 'Long JABACABA..., Second J'); // Search for a non-ASCII string in a pure ASCII string.
  ...
  var allCharsBufferUcs2 = new Buffer(allCharsString, 'ucs2'); // Search for string long enough to trigger complex search with ASCII pattern
  // and UC16 subject.
  ...
  t.equal(new Buffer('aaaa').indexOf('你好', 'ucs2'), -1); // Haystack has odd length, but the needle is UCS2.
