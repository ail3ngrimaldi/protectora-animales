      ...
      TSNonNullExpression: function TSNonNullExpression(node) {
        ...
        function convertTokenToOptional(replacement) {
          return function (fixer) {
            var operator = sourceCode.getTokenAfter(node.expression, util.isNonNullAssertionPunctuator);
        ...
        if (node.parent) {
          if ((node.parent.type === experimental_utils_1.AST_NODE_TYPES.MemberExpression || node.parent.type === experimental_utils_1.AST_NODE_TYPES.OptionalMemberExpression) && node.parent.object === node) {
          ...
          } else if ((node.parent.type === experimental_utils_1.AST_NODE_TYPES.CallExpression || node.parent.type === experimental_utils_1.AST_NODE_TYPES.OptionalCallExpression) && node.parent.callee === node) {
            if (!node.parent.optional) {
              // it is x.y?.z!()
              suggest.push({
                messageId: 'suggestOptionalChain',
