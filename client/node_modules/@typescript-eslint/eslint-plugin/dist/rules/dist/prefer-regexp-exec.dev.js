...
exports["default"] = util_1.createRule({
  ...
  create: function create(context) {
    ...
    return {
      "CallExpression[arguments.length=1] > MemberExpression.callee[property.name='match'][computed=false]": function CallExpressionArgumentsLength1MemberExpressionCalleePropertyNameMatchComputedFalse(node) {
        var callNode = node.parent;
        var arg = callNode.arguments[0];
        var evaluated = util_1.getStaticValue(arg, globalScope); // Don't report regular expressions with global flag.

        if (evaluated && evaluated.value instanceof RegExp && evaluated.value.flags.includes('g')) {
          return;
        }
