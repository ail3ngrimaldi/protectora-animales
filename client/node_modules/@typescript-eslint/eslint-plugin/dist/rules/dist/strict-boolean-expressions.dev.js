...
exports["default"] = util.createRule({
  ...
  create: function create(context, _ref) {
    ...
    function checkNode(node) {
      ...
      checkedNodes.add(node); // for logical operator, we also check its operands

      if (node.type === experimental_utils_1.AST_NODE_TYPES.LogicalExpression && node.operator !== '??') {
        var hasError = false;
      ...
      var messageId;
      var types = inspectVariantTypes(tsutils.unionTypeParts(type));
      ...
      var is = function is() {
        ...
        return types.size === wantedTypes.length && wantedTypes.every(function (type) {
    ...
    function inspectVariantTypes(types) {
      ...
      if (types.some(function (type) {
        return !tsutils.isTypeFlagSet(type, ts.TypeFlags.Null | ts.TypeFlags.Undefined | ts.TypeFlags.VoidLike | ts.TypeFlags.BooleanLike | ts.TypeFlags.StringLike | ts.TypeFlags.NumberLike | ts.TypeFlags.Any | ts.TypeFlags.Unknown | ts.TypeFlags.Never);
      })) {
